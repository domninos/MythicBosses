...
public class MessagesUtil {
    ...
    public void load() {
        ...
        List<String> preAnnounceMsg = plugin.getConfigHandler().getConfig().getStringList("pre_announcement");
        this.pre_announcement = plugin.translate(StringUtils.join(preAnnounceMsg, "\n"));

        List<String> bossSpawnMsg = plugin.getConfigHandler().getConfig().getStringList("bossSpawn");
        this.bossSpawn = plugin.translate(StringUtils.join(bossSpawnMsg, "\n"));

        List<String> bossDeathMsg = plugin.getConfigHandler().getConfig().getStringList("bossDeath");
        this.bossDeath = plugin.translate(StringUtils.join(bossDeathMsg, "\n"));

        List<String> rewardedPlayersMsg = plugin.getConfigHandler().getConfig().getStringList("rewardedPlayers");
        this.rewardedPlayers = plugin.translate(StringUtils.join(rewardedPlayersMsg, "\n"));
    }

    public String getPreAnnouncement(String bossName) {
        return pre_announcement.replace("%boss%", bossName);
    }
    ...
    public String getRewardedPlayers(String bossName, String top_1_damager, String top_2_damager, String top_3_damager) {
        return rewardedPlayers.replace("%boss%", bossName).
                replace("%top_1_damager%", top_1_damager).
                replace("%top_2_damager%", top_2_damager).
                replace("%top_3_damager%", top_3_damager);
    }
